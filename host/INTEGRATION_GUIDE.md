# Tasify Native Messaging Host — Integration Guide
...
Claude Code (HTTP POST) → Native Host (Express) → Chrome Extension (Stdio)
Chrome Extension (Stdio) → Native Host (Shell Exec) → Claude Code (CLI)
...
The host will start on port 3000 by default. Override with TASIFY_PORT env var.

## Testing

### Test HTTP→Stdio forwarding
...
|---|---|---|
| TASIFY_PORT | 3000 | HTTP listener port for Claude Code hooks |
...
This applies to both directions (Host → Browser and Browser → Host).

### Supported commands (Browser → Host)
